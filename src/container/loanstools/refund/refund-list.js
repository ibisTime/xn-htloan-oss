...
class refundList extends React.Component {
    ...
    componentDidMount(): void {
        ...
        let btnArr = {
            parentCode: 'SM201812191857341993291',
    ...
    getAccessSlip = (pageNumber) => {
        ...
        accessSlipNotAdvance(pageNumber, 4, code === '' ? '' : code, customerName === '' ? '' : customerName).then(data => {
            let arr = [];
            for(let i = 0; i < data.list.length; i++) {
                arr.push({
                    ...
                    // 状态
                    curNodeCode: data.list[i].backAdvanceStatus ? data.list[i].backAdvanceStatus : '',
                    // 贷款金额
    ...
    cpMg1 = () => {
        if(this.checkBoxGroup.length <= 0) {
            ...
            this.props.history.push(`/finance/refund/certain?code=${this.checkBoxGroup[0].split('|')[0]}`);
        }
    }
    // 打件
    cpMg2 = () => {
        ...
        return (
            <div className="preLoan-access-list-global">
                ...
                <div className="preLoan-access-list-btn-group">
                    {
                        rationaleOk ? (
                            <span className="preLoan-access-list-btn-gray" onClick={this.cpMg1} style={{marginRight: '30px', width: '100px'}}>财务确认退款</span>
                        ) : null
                    }
                    {
                        detail ? (
