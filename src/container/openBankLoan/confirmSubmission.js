...
class confirmSubmission extends React.Component {
    ...
    sendSave = () => {
        ...
        let arr = {
            code: this.code,
            bankFkDatetime: iptArr.time,
            bankFkNote: iptArr.rmk
        };
        sendBankAmount(arr).then(data => {
            showSucMsg('操作成功');
            setTimeout(() => {
    ...
    render() {
        ...
        return (
            <div className="afp-body">
                ...
                <Row style={{marginTop: '20px'}}>
                    <Col span={12}>提交时间：<input type="text" value={iptArr.time} ref={input => this.timeIpt = input} onChange={(e) => { this.iupChange(e, 'time'); }} className="dealer-user-detail-edit-input" /><span style={{color: '#999999'}}>（默认当前时间）</span></Col>
                    <Col span={12}></Col>
                </Row>
                <Row style={{marginTop: '20px'}}>
                    <Col span={2}>提交说明：</Col>
