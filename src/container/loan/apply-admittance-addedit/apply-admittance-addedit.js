...
class ApplyAdmittanceAddEdit extends React.Component {
    ...
    render() {
        let creditUserListData = this.props.pageData ? this.props.pageData.creditUserList : '';
        let creditUserData = {};

        if (creditUserListData) {
            creditUserListData.map((item) => {
                if (item.relation === '1') {
                    creditUserData = item;
                    return false;
                }
            ...
            items: [
                [{
                    ...
                    formatter: function (v, d) {
                        return creditUserData.userName;
                    }
                }],
                [{
                    field: 'gender',
