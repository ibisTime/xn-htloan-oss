            ...
            <Card style={{marginTop: 16}} title={title + '数据'}>
                <Row gutter={54}>
                    {this.getRangeDateCol({
                        field: fields[0],
                        title: '流水时间',
                        onChange: (dates) => this.jourDatetimeChange(dates, fields[7], fields[8], fields[10], fields[11]),
                        type: 'date',
                ...
                <Row gutter={54}>
                    {this.getInputCol({
                        field: fields[7],
                        title: '总收入(元)',
                        onChange: (v) => this.moneyChange(v, fields[0], fields[10]),
                        amount: true
                    }, 2)}
                    {this.getInputCol({
                        field: fields[8],
                        title: '总支出(元)',
                        onChange: (v) => this.moneyChange(v, fields[0], fields[11]),
                        amount: true
                    }, 2)}
                </Row>
                <Row gutter={54}>
            ...
            <Spin spinning={this.state.fetching}>
                <Form>
                    <Tabs defaultActiveKey="1">
                        <TabPane tab="贷款车辆信息" key="1"><Card title="贷款车辆信息">
                            ...
                            {this.checkCommissionerTwo || !isUndefined(pageData.carPriceCheckReport) ? (
                                <Row gutter={54}>
                                    ...
                                    {this.getFileCol({
                                        field: 'carPriceCheckReport',
                                        title: '车辆价格核实报告',
                                        type: 'img',
                                        required: true
                                    }, this.checkCommissionerTwo ? 2 : 1)}
                                </Row>
                            ) : null}
                        </Card></TabPane>
                        ...
                        <TabPane tab="流转日志" key="5"><Card style={{marginTop: 16}} title="流转日志">
                            <Table {...this.getTableProps()} />
                        </Card></TabPane>
                            <TabPane tab="配偶" key="6"> <Card style={{marginTop: 16}} title="配偶信息">
                                <Row gutter={54}>
                        ...
                        <TabPane tab="担保人信息" key="7">   <Card style={{marginTop: 16}} title="担保人信息">
                            <Row gutter={54}>
                    ...
                    {!this.view && !showSqryhls ? (
                        <Button onClick={() => this.setState({showSqryhls: true})}
                                style={{width: '100%', marginTop: 10}} type="dashed">
                            <Icon type="plus"/>新增 申请人银行流水
                        </Button>
                    ) : null}
                    {!this.view && !showSqrzfbls ? (
                        <Button onClick={() => this.setState({showSqrzfbls: true})}
                                style={{width: '100%', marginTop: 10}} type="dashed">
                            <Icon type="plus"/>新增 申请人支付宝流水
                        </Button>
                    ) : null}
                    {!this.view && !showSqrwxls ? (
                        <Button onClick={() => this.setState({showSqrwxls: true})}
                                style={{width: '100%', marginTop: 10}} type="dashed">
                            <Icon type="plus"/>新增 申请人微信流水
                        </Button>
                    ) : null}
                    {!this.view && !showPoyhls ? (
                        <Button onClick={() => this.setState({showPoyhls: true})} style={{width: '100%', marginTop: 10}}
                                type="dashed">
                            <Icon type="plus"/>新增 配偶银行流水
                        </Button>
                    ) : null}
                    {!this.view && !showPozfbls ? (
                        <Button onClick={() => this.setState({showPozfbls: true})}
                                style={{width: '100%', marginTop: 10}} type="dashed">
                            <Icon type="plus"/>新增 配偶支付宝流水
                        </Button>
                    ) : null}
                    {!this.view && !showPowxls ? (
                        <Button onClick={() => this.setState({showPowxls: true})} style={{width: '100%', marginTop: 10}}
                                type="dashed">
                            <Icon type="plus"/>新增 配偶微信流水
                        </Button>
                    ) : null}
                    {!this.view && !showDbryhls ? (
                        <Button onClick={() => this.setState({showDbryhls: true})}
                                style={{width: '100%', marginTop: 10}} type="dashed">
                            <Icon type="plus"/>新增 担保人银行流水
                        </Button>
                    ) : null}
                    {!this.view && !showDbrzfbls ? (
                        <Button onClick={() => this.setState({showDbrzfbls: true})}
                                style={{width: '100%', marginTop: 10}} type="dashed">
                            <Icon type="plus"/>新增 担保人支付宝流水
                        </Button>
                    ) : null}
                    {!this.view && !showDbrwxls ? (
                        <Button onClick={() => this.setState({showDbrwxls: true})}
                                style={{width: '100%', marginTop: 10}} type="dashed">
                        ...
                        {this.isCheck()
                            ? <div>
                                ...
                                <Button style={{marginLeft: 20}} type="primary"
                                        onClick={() => this.checkInfo(0)}>不通过</Button>
                            ...
                            : !this.view
                                ? <div>
                                    ...
                                    <Button style={{marginLeft: 20}} type="primary"
                                            onClick={() => this.checkForm(1)}>提交</Button>
                                    <Button style={{marginLeft: 20}} onClick={this.onCancel}>返回</Button>
                                </div>
                                : <Button style={{marginLeft: 20}} onClick={this.onCancel}>返回</Button>
                        }
