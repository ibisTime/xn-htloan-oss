...
import {Row, Col, Checkbox, Pagination, DatePicker} from 'antd';
import {
    myBusinessPage,
    accessSlipStatus,
    showButton
...
class wdyw extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            accessSlipList: [],
    ...
    componentDidMount(): void {
        ...
        let btnArr = {
            parentCode: 'SM201904151542329069697',
        ...
        showButton(btnArr).then(data => {
            ...
            for(let i = 0; i < data.length; i++) {
                if(data[i].url === '/ywcx/ywcx/details') {
                    this.setState({
                        detail: true
                    });
                }
    ...
    getAccessSlip = (pageNumber) => {
        const {code, customerName, applyDatetimeStart, applyDatetimeEnd} = this.state;
        myBusinessPage(pageNumber, 4, code === '' ? '' : code, customerName === '' ? '' : customerName, applyDatetimeStart, applyDatetimeEnd).then(data => {
            let arr = [];
            for(let i = 0; i < data.list.length; i++) {
                arr.push({
                    ...
                    // 身份证
                    idNo: data.list[i].creditUser ? data.list[i].creditUser.idNo : '',
                    // 手机号
                    mobile: data.list[i].creditUser ? data.list[i].creditUser.mobile : ''
                });
            }
            this.setState({
                accessSlipList: [...arr],
                        ...
                        : accessSlipList.map(d => {
                            return (<Row className="preLoan-access-list-item">
                                <Col span={1} style={{lineHeight: '130px'}}>
                                    <Checkbox
                                        value={d.code + '|' + d.curNodeCode} onChange={this.onChange}
                                        checked={
                                ...
                                <Col span={23}>
                                    ...
                                    <Row style={{marginTop: '23px'}}>
                                        ...
                                        <Col span={5}>身份证：{d.idNo}</Col>
                                        <Col span={5}>手机号：{d.mobile}</Col>
                                        <Col span={5}>贷款银行：{d.loanBankName}</Col>
                                        <Col span={4}>贷款金额：{d.loanAmount / 1000}</Col>
                                    </Row>
                                    <Row style={{marginTop: '15px'}}>
                                        <Col span={5}>贷款期数：{d.periods}期</Col>
                                        <Col span={5}>是否垫资：{d.isAdvanceFund === '0' ? '否' : '是'}</Col>
                                        <Col span={5}>业务公司：{d.saleUserCompanyName}</Col>
                                        <Col span={5}>业务团队：{d.teamName}</Col>
                                        <Col span={4}>业务员：{d.saleUserName}</Col>
                                    </Row>
                                </Col>
                            </Row>);
                        })
