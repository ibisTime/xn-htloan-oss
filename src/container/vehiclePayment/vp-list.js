...
import {Row, Col, Checkbox, Pagination, Select, Modal, Button, DatePicker} from 'antd';
import {
    accessSlipNotFinance,
    accessSlipStatus,
    showButton
} from '../../api/preLoan.js';
...
class refundList extends React.Component {
    ...
    handleOk = (value) => {
        const {regDate, regDate2} = this.state;
        if(value === 'model1') {
            this.setState({
                visible: false
            });
    ...
    componentDidMount(): void {
        ...
        let btnArr = {
            parentCode: 'SM201912051358333226186',
        ...
        showButton(btnArr).then(data => {
            ...
            for(let i = 0; i < data.length; i++) {
                if(data[i].url === '/payment') {
                    this.setState({
                        rationaleOk: true
                    });
                }else if(data[i].url === '/takeBack') {
                    this.setState({
                        detail: true
                    });
                }else if(data[i].url === '/details') {
                    this.setState({
                        details: true
                    });
                }
        ...
        accessSlipNotFinance(pageNumber, 4, code === '' ? '' : code, customerName === '' ? '' : customerName, ['0', '1', '2']).then(data => {
            ...
            for(let i = 0; i < data.list.length; i++) {
                arr.push({
                    ...
                    // 状态
                    financeStatus: data.list[i].financeStatus ? data.list[i].financeStatus : '',
                    // 贷款金额
    ...
    cpMg1 = () => {
        ...
        }else if(this.checkBoxGroup[0].split('|')[1] != '0') {
            showWarnMsg('当前状态无权操作');
        }else{
            // this.checkBoxGroup[0].split('|')[0]
            this.showModal('model1');
        }
    }
    sendDetail = () => {
        if(this.checkBoxGroup.length <= 0) {
            ...
            showWarnMsg('请选择不大于一条记录');
        }else if(this.checkBoxGroup[0].split('|')[1] != '1') {
            showWarnMsg('当前状态无权操作');
        }else {
            // this.checkBoxGroup[0].split('|')[0]
            this.showModal('model2');
        }
    }
    sendDetails = () => {
        if(this.checkBoxGroup.length <= 0) {
            ...
            // this.checkBoxGroup[0].split('|')[0]
            this.props.history.push(`/loan/advMoney/detailPrint?code=${this.checkBoxGroup[0].split('|')[0]}`);
        }
    }
    // 打件
    cpMg2 = () => {
        ...
        return (
            <div className="preLoan-access-list-global">
                ...
                <div className="preLoan-access-list-btn-group">
                    {
                        rationaleOk ? (
                            <span className="preLoan-access-list-btn-gray" onClick={this.cpMg1} style={{marginRight: '30px'}}>财务支付</span>
                        ) : null
                    }
                    {
                        detail ? (
                            <span className="preLoan-access-list-btn-gray" onClick={this.sendDetail} style={{marginRight: '30px'}}>财务收回</span>
                        ) : null
                    }
                    {
                        details ? (
                            <span className="preLoan-access-list-btn-gray" onClick={this.sendDetails} style={{marginRight: '30px'}}>详情</span>
                        ) : null
                    }
                    <div className="clear"></div>
                </div>
                        ...
                        : accessSlipList.map(d => {
                            return (<Row className="preLoan-access-list-item">
                                <Col span={1} style={{lineHeight: '130px'}}>
                                    <Checkbox value={d.code + '|' + d.financeStatus} onChange={this.onChange} />
                                </Col>
                                <Col span={23}>
                                    <Row>
                                        ...
                                        <Col span={17} style={{textAlign: 'right'}}>
                                            <span>状态：{d.financeStatus === '0' ? '待支付' : (d.financeStatus === '1' ? '已支付待收回' : '已收回')}</span>
                                        </Col>
                                    </Row>
                ...
                <Modal
                    ...
                    onOk={value => this.handleOk('model1')}
                    onCancel={value => this.handleCancel('model1')}
                >
                    <p><span style={{color: 'red', marginLeft: '20px'}}>* </span>操作时间:</p>
                    <p className="ant-modal-body" style={{height: '100px'}}>
                ...
                <Modal
                    ...
                    onOk={value => this.handleOk('model2')}
                    onCancel={value => this.handleCancel('model2')}
                >
                    <p><span style={{color: 'red', marginLeft: '20px'}}>* </span>操作时间:</p>
                    <p className="ant-modal-body" style={{height: '100px'}}>
                        <DatePicker format={'YYYY-MM-DD'} defaultValue={moment(new Date(), 'YYYY-MM-DD')} style={{width: '220px', float: 'left'}} onChange={this.onChangeTime}/>
